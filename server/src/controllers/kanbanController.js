...
import * as kanbanRepository from '../repository/kanbanRepository.js';
...
export const getAllKanbans = async (req, res, next) => {
  try {
    res.data.kanbans = await kanbanRepository.getAllKanbans();
    next();
  } catch (error) {
    return next(error);
  }
...
export const getKanbanById = async (req, res, next) => {
  try {
    const kanban = await kanbanRepository.getKanbanById(req.params.id);
...
export const updateKanban = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { title, description, stages } = req.body;
    await kanbanRepository.updateKanban(id, { title, description, stages });
